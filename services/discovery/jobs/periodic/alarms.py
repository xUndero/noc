    ...
    def handler(self):
            ...
            for new_event in create_objects_events:
                if "object_id" in object_events[new_event]:
                    managed_object = self.find_cpe(
                        object_events[new_event]["object_id"], self.object.id
                    )
                    if not managed_object:
                        self.logger.warning(
                            "No object %s for alarm: \n%s"
                            % (object_events[new_event]["object_id"], object_events[new_event])
                        )
                        continue
                else:
        ...
        if close_objects_events:
            ...
            for close_event in close_objects_events:
                ...
                self.raise_event(
                    self.logger, event.managed_object.id, event.managed_object.pool.name, raw_vars
                )
