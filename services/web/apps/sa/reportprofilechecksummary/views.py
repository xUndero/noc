...
class ReportFilterApplication(SimpleReport):
    ...
    def get_data(self, request, report_type=None, **kwargs):
        ...
        r_map = [
            ...
            (_("Is Managed, object type defined bad CLI Credential"), "2is1.6is1.3isp0.2isp1"),
        ...
        for p in Pool.objects.filter().order_by("name"):
            ...
            data += [
                (sc, x, y, self.calc_percent(x, y), url % (columns[columns_desr.index(x)], p.name))
        ...
        data += [
            (sc, x, y, self.calc_percent(x, y), url % (columns[columns_desr.index(x)], ""))
