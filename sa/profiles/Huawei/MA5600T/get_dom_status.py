...
class Script(BaseScript):
    ...
    def execute_snmp(self, interface=None, **kwargs):
        r = []
        names = {v: k for k, v in six.iteritems(self.scripts.get_ifindexes(name_oid="IF-MIB::ifName"))}
        for olt_index, olt_temp_c, olt_voltage_v, olt_current_ma, olt_optical_tx_dbm,\
            olt_optical_rx_dbm in self.snmp.get_tables([
            ...
            r += [{"interface": names[int(iface_index)],
                   ...
                   "optical_rx_dbm": float(olt_optical_rx_dbm) / 100.0 if olt_optical_rx_dbm != 2147483647 else 0}]
        return r
